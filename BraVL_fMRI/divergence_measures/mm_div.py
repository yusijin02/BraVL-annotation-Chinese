...
def poe(mu, logvar, eps=1e-8):
    # POE算法, 将多个高斯分布融合为一个高斯分布
    # 输入mu和logvar都是(模态数+1, batch_size, 32)形状, 且最后一个模态都是全0
    # 加入一个额外模态, 均值0, 方差1, 是标准正态分布
    var = torch.exp(logvar) + eps  # 将对数方差计算为方差, eps为平滑项, 防止为0
    # 现在var一定是正数, 这也是预测对数方差的目的
    # precision of i-th Gaussian expert at point x
    T = 1. / var  # 每个模态的权重(第i个一维高斯分布的精度)
    pd_mu = torch.sum(mu * T, dim=0) / torch.sum(T, dim=0)  # 融合后的均值
    pd_var = 1. / torch.sum(T, dim=0)  # 融合后的方差
    pd_logvar = torch.log(pd_var)  # 融合后的对数方差
    return pd_mu, pd_logvar  # 输出的是融合后的高斯分布均值和对数方差, 形状都是(batch_size, 32)
...
def calc_group_divergence_moe(flags, mus, logvars, weights, normalization=None):
    ...
    # weights的形状是(模态数,)
    num_mods = mus.shape[0]     # 模态组合数
    num_samples = mus.shape[1]  # 样本数
    if normalization is not None:
        klds = torch.zeros(num_mods)
    else:
        klds = torch.zeros(num_mods, num_samples)
    ...
    for k in range(0, num_mods):  # 对于每个模态组合
        # 计算和标准方差之间的KL散度
        kld_ind = calc_kl_divergence(mus[k,:,:], logvars[k,:,:], norm_value=normalization)  # 返回一个标量
        # 这里一共有batch_size个32维高斯分布, kld_ind计算的是这batch_size个32维高斯分布与标准的32维高斯分布之间的KL散度
        # norm_value将传入一个batch_size, 这样kld_ind就是在计算batch_size个高斯分布的KL散度的平均值
        if normalization is not None:
            klds[k] = kld_ind
        else:
            klds[k,:] = kld_ind
    if normalization is None:
        weights = weights.unsqueeze(1).repeat(1, num_samples)
    group_div = (weights*klds).sum(dim=0)
    # group_div形状是(batch_size,), 表示n组模态组合之间样本的KL散度和(没传递归一化参数)或样本的归一化KL散度和(传递了归一化参数)
    # klds形状是(n,)或(n,batch_size), 表示n组模态组合分别的样本的KL散度和(没传递归一化参数)或样本的归一化KL散度和(传递了归一化参数)
